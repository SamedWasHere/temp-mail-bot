...
def welcome(message):
    bot.reply_to(message, "🚀 Bot Aktif! /yeni yazarak yeni bir mail alabilirsin.")
...
def new_mail(message):
    try:
        ...
        if res.status_code == 200:
            ...
            bot.reply_to(message, f"⚠️ Mail servisi geçici olarak reddetti. Hata: {res.status_code}")
    except Exception as e:
        bot.reply_to(message, f"❌ Teknik Hata: {str(e)}")

# --- ÇAKIŞMAYI (409) TEMİZLEYEN BAŞLANGIÇ ---
if __name__ == "__main__":
    # Eski tüm bağlantıları (webhook) temizler
    bot.remove_webhook()
    time.sleep(1)
